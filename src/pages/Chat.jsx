...
import {
    FaPaperPlane, FaSyncAlt, FaTimes, FaImage, FaHistory, FaTrashmer
    ,
    FaTrophy, FaChartLine, FaLayerGroup, FaWaveSquare,
    FaClock, FaSignOutAlt, FaMedal, FaBrain, FaSearch, FaChevronDown, FaPlus, FaTrash,
...
export default function Chat() {
    ...
    useEffect(() => {
        if (!currentUser || !currentSessionId || !userData) return;
    ...
    }, [userData?.board, userData?.classLevel, userData?.class, currentUser, currentSessionId]);
    ...
    const syncContext = async (overrideSubj, overrideChap, overrideTitle) => {
        ...
        const userClass = userData?.classLevel || userData?.class;

        if (!currentSessionId || !userClass) {
        ...
        try {
            ...
            if (overrideSubj === undefined && overrideChap === undefined && overrideTitle === undefined) {
                toast.success(`Synced: ${userData.board} Class ${userClass}`);
    ...
    const sendMessage = async (override = null) => {
        ...
        // Standardize userClass
        const userClass = userData?.class || userData?.classLevel;

        const userLang = userData.language;

        const systemInstruction = userLang === 'Hinglish' ? `You are a Dhruva AI Tutor for Class ${userClass}. Respond in Hinglish (a natural mix of Hindi and English). Use English for technical terms and Hindi for explanations.` : `You are a Dhruva AI Tutor for Class ${userClass}. Respond in formal English.`;
        ...
        formData.append("class", userData.class || userData.classLevel);
        ...
        try {
            console.log("SENDING TO AI -> Board:", userData.board, "Class:", userData.classLevel || userData.class);
