...
import { db } from "../firebase"; // Adjusted to standard path
...
export default function Profile() {
    ...
    // profileData state matches registration fields
    ...
    const avatarStyles = ["adventurer", "avataaars", "bottts", "big-smile", "miniavs", "open-peeps", "pixel-art", "lorelei", "micah", "notionists", "rings", "shapes"];
            ...
            try {
                ...
                if (docSnap.exists()) {
                    ...
                    setProfileData({
                        ...
                        board: d.board || "CBSE",
                        classLevel: d.classLevel || d.class || "10", // Fallback for old "class" key
    ...
    const handleSave = async () => {
        ...
        setLoading(true);
        try {
            // Update Firestore with keys matching Register.jsx
    ...
    return (
        <div className={`min-h-screen transition-colors duration-700 relative pb-20 ${s.bg}`}>
            ...
            <main className="max-w-4xl mx-auto pt-16 sm:pt-24 px-4 sm:px-6 relative z-10">
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className={`rounded-[40px] sm:rounded-[60px] p-6 sm:p-16 border shadow-2xl ${s.card}`}>
                    
                    {/* Header Section */}
                    <div className="flex flex-col items-center sm:flex-row gap-8 sm:gap-12 mb-16">
                        <div className="relative group">
                            ...
                            <div className="relative p-1 rounded-[38%] bg-gradient-to-br from-indigo-500 via-purple-500 to-fuchsia-500">
                                ...
                                <button
                                    ...
                                    className="absolute -bottom-2 -right-2 bg-indigo-600 text-white p-3 sm:p-4 rounded-2xl shadow-xl hover:scale-110 active:scale-95 transition-all"
                                >
                        ...
                        <div className="text-center sm:text-left">
                            <h1 className="text-4xl sm:text-7xl font-black italic uppercase tracking-tighter mb-2 leading-tight">User<span className={s.accent}>.</span>Meta</h1>
                    ...
                    <div className="space-y-8 sm:space-y-12">
                        ...
                        <div className="relative">
                            ...
                            <div className={`grid grid-cols-3 gap-3 sm:gap-5 p-2 sm:p-3 rounded-[28px] border ${theme === 'dark' ? 'border-white/10 bg-white/[0.02]' : 'border-slate-200 bg-slate-100/30'}`}>
                                {[
                                    ...
                                    <button key={g.id} type="button" onClick={() => setProfileData({ ...profileData, gender: g.id })}
                                        className={`flex flex-col items-center py-4 sm:py-6 rounded-[20px] transition-all duration-300 border ${profileData.gender === g.id ? `${theme === 'dark' ? 'bg-white/10 border-white/20' : 'bg-white border-white shadow-md'} scale-[1.02]` : 'opacity-20 border-transparent'}`}
                                    >
                                        <span className={`text-2xl sm:text-3xl mb-1 ${profileData.gender === g.id ? g.color : ''}`}>{g.icon}</span>
                    ...
                    <motion.button whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.98 }} onClick={handleSave} disabled={loading}
                        className={`w-full mt-12 py-6 rounded-[24px] text-white font-black text-xs uppercase tracking-[0.5em] shadow-2xl flex items-center justify-center gap-4 ${s.btn}`}
                    >
                    ...
                    <div className="mt-16 pt-10 border-t border-white/5">
                        <div className="flex items-center gap-4 mb-8">
                            <FaShieldAlt className={`${hasPassword ? 'text-red-500' : 'text-green-500'} text-lg`} />
                            <h3 className="text-[10px] font-black uppercase tracking-[0.4em] opacity-40">{hasPassword ? "Security Override" : "Establish Password Node"}</h3>
                        </div>
                        <div className="space-y-4">
                            {hasPassword && (
                                <input type="password" placeholder="CURRENT KEY" value={passwords.oldPass} onChange={(e) => setPasswords({ ...passwords, oldPass: e.target.value })}
                                    className={`w-full p-5 rounded-[20px] border outline-none text-[10px] font-black tracking-widest ${s.input}`} />
                            )}
                            <div className="relative">
                                <input type={showPass.new ? "text" : "password"} placeholder={hasPassword ? "NEW SECURITY HASH" : "CREATE NEW KEY"} value={passwords.newPass} onChange={(e) => setPasswords({ ...passwords, newPass: e.target.value })}
                                    className={`w-full p-5 rounded-[20px] border outline-none text-[10px] font-black tracking-widest ${s.input}`} />
                            ...
                            <button onClick={handlePasswordUpdate} disabled={loading} className={`w-full py-4 rounded-[20px] transition-all flex items-center justify-center gap-3 text-[10px] font-black uppercase tracking-widest ${hasPassword ? 'bg-red-500/10 border border-red-500/20 text-red-500 hover:bg-red-500 hover:text-white' : 'bg-indigo-500/10 border border-indigo-500/20 text-indigo-500 hover:bg-indigo-500 hover:text-white'}`}>
                                {loading ? <FaSyncAlt className="animate-spin" /> : <FaKey />} {hasPassword ? "Flash_Auth_Memory" : "Initialize_Key_Link"}
                            </button>
                        </div>
                    </div>
                    ...
                    <div className="mt-12 pt-8 border-t border-white/5">
                        <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => navigate("/chat")}
                            className={`group relative w-full flex items-center p-2 pr-10 rounded-[24px] bg-transparent border-2 transition-all duration-300 ${theme === 'dark' ? 'border-white/10 hover:border-indigo-500/50' : 'border-slate-200 hover:border-blue-500/50'}`}
                        >
                            <div className={`${theme === 'dark' ? 'bg-white/5' : 'bg-slate-100'} group-hover:bg-indigo-600 p-4 rounded-[18px] transition-all duration-300`}>
                                <FaArrowLeft className={`${theme === 'dark' ? 'text-white' : 'text-slate-600'} group-hover:text-white text-lg transition-transform group-hover:-translate-x-1`} />
                            </div>
                            <div className="text-left ml-5">
                                <span className={`block font-black text-[11px] uppercase tracking-[0.4em] mb-1.5 ${theme === 'dark' ? 'text-white' : 'text-slate-800'} group-hover:text-indigo-500`}>Return To Chat</span>
                                <span className="text-[8px] opacity-40 font-black uppercase tracking-[0.2em]">Close_Config // Exit_Node</span>
                            </div>
                        </motion.button>
                    </div>
                </motion.div>
